...
- ファイルアップロードでログを取り込み
- 対応フォーマット（8種類のログタイプ）:
  - Webアクセスログ (Apache/Nginx形式) - 1,000件
  - アプリケーションログ (JSON形式) - 1,000件
  - セキュリティログ (syslog形式) - 1,000件
  - JVM GCログ (G1GC/ParallelGC形式) - 1,000件
  - Kubernetesログ (コンテナログ形式) - 1,000件
  - データベースログ (MySQL slow query形式) - 1,000件
  - ネットワーク/Firewallログ (Firewall形式) - 1,000件
  - システムメトリクスログ (Metrics形式) - 1,000件
- サンプルデータを初回起動時に自動読み込み（合計8,000件）
- ユーザーによるデータ追加も可能
