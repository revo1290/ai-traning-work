...
- SPLクエリ入力エリア
  - 構文エラーのハイライト表示
  - コマンド入力時のヘルプポップアップ（実装済み: ヘルプモーダル）
  - SPLテンプレート・例文集へのアクセス（実装済み: 40+テンプレート）
- 時間範囲セレクター（実装済み: 全期間〜30日のプリセット）
- 検索結果テーブル（実装済み: ページネーション対応）
- フィールドサイドバー（抽出されたフィールド一覧）
- 検索結果の可視化オプション
- エクスポートボタン（CSV/JSON）- 実装済み
- 検索の保存ボタン - 実装済み
- クエリ履歴パネル - 実装済み
