...
#### 四則演算サポート
- eval式での四則演算（`+`, `-`, `*`, `/`, `%`）を完全サポート
- 演算子の優先順位を正しく処理（乗除が加減より優先）
- 例: `eval total = price * quantity + tax`
...
#### UI機能追加
- 時間範囲セレクター（全期間、15分〜30日のプリセット）
- SPLテンプレート集（40+テンプレート、7カテゴリ）
- SPLコマンドヘルプ（全コマンドの構文・例・説明）
